...
// In case publish place Janus behind an Apache frontend (as we did on the
...
if(window.location.protocol === 'http:')
	...
	server = "https://yiqi.jasconcept.com:8089/janus";
*/
var uv4ldata = null;
var janus = null;
var mysql_token = null;
var mysql_room = null;
var mysql_pin = null;
var sfutest = null;
...
$(document).ready(function() {
	...
	Janus.init({debug: "all", callback: function() {
		let authstring = window.location.search.substr(1).split("==");
		// Prepare posting auth. string
		const formData = new FormData()
		formData.append("authstring", authstring[0]+"==");
		formData.append("value", authstring[1]);
		// Display the key/value pairs
		//for(var pair of formData.entries()) { console.log(pair[0]+ ', '+ pair[1]); }
		axios.post('/auth',formData,{ headers: { 'Content-Type':'multipart/form-data' } })
		.then((response) => {
			console.log(response.data.auth)
				if (response.data.auth == "True") {
					mysql_token = response.data.token;
					mysql_room = response.data.room;
					mysql_pin = response.data.pin;
					mysql_username = response.data.username;
									...
									janus.attach(
										{
											...
											onmessage: function(msg, jsep) {
												...
												Janus.debug(msg);
												// Janus.log("OOOOOOOOOOOOO "+JSON.stringify(msg));
												...
												if(event != undefined && event != null) {
													if(event === "joined") {
														...
														Janus.log("Who is talkin?? " + talkingid);
														var remoteFeed = null;
														for(var i=1; i<6; i++) {
															if(feeds[i] != null && feeds[i] != undefined && feeds[i].rfid == talkingid) {
																remoteFeed = feeds[i];
																break;
															}
														}
														$('#remote'+remoteFeed.rfindex).addClass("talking");
													} else if(event === "stopped-talking") {
														// Who is NOT talkin??
														var talkingid = msg["id"];
														Janus.log("Who is NOT talkin?? " + talkingid);
														var remoteFeed = null;
														for(var i=1; i<6; i++) {
															if(feeds[i] != null && feeds[i] != undefined && feeds[i].rfid == talkingid) {
																remoteFeed = feeds[i];
														...
														$('#remote'+remoteFeed.rfindex).removeClass("talking");
													} else if(event === "event") {
														// Any new feed to attach to?
														if(msg["publishers"] !== undefined && msg["publishers"] !== null) {
															var list = msg["publishers"];
											...
											onlocalstream: function(stream) {
												...
												if($('#myvideo').length === 0) {
													...
													// Add a 'mute' button
													$('#videolocal').append('<button class="btn btn btn-outline-danger btn-md" id="mute" style="position: absolute; bottom: 26px; left: 21px;"><span class="fa fa-microphone"></span></button>');
													$('#mute').click(toggleMute);
													// Add an 'unpublish' button
													$('#videolocal').append('<button class="btn btn btn-outline-danger btn-md" id="unpublish" style="position: absolute; bottom: 26px; right: 21px;"><span class="fa fa-eye-slash"></span></button>');
													$('#unpublish').click(unpublishOwnFeed);
												}
												$('#publisher').removeClass('hide').html(mysql_username).show();
		...
		}); // button related curly
				} // axios post related curlu
			}, 
		(error) => {
			console.log(error);
		});
...
function toggleMute() {
	...
	$('#mute').html(muted ? "<span class=\"fa fa-microphone-slash\"></span>" : "<span class=\"fa fa-microphone\"></span>");
}
	...
	janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				var subscribe = { "request": "join", "room": myroom, "pin": myroompin, "ptype": "listener", "feed": id, "private_id": mypvtid };
			...
			onmessage: function(msg, jsep) {
				...
				if(msg["error"] !== undefined && msg["error"] !== null) {
					...
					if(event === "attached") {
						...
						// return the part of this artificially created string
						...
						$('#feed'+remoteFeed.rfindex).append(' <span class="label label-default hide" id="'+remoteFeed.rfid+'"></span>').show();
					} else if(event === "event") {
